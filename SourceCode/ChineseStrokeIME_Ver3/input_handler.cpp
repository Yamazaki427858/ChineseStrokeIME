...
// 獲取當前焦點的編輯控件
static HWND GetFocusedEditControl() {
    HWND hForeground = GetForegroundWindow();
    ...
    if (hFocus && IsWindow(hFocus)) {
        ...
        if (wcscmp(className, L"Edit") == 0 ||
            wcscmp(className, L"Scintilla") == 0 ||  // Notepad++ 使用 Scintilla
    ...
    // 如果沒有焦點控件，嘗試查找子編輯控件
    HWND hEdit = FindWindowEx(hForeground, NULL, L"Edit", NULL);
    if (!hEdit) {
        hEdit = FindWindowEx(hForeground, NULL, L"Scintilla", NULL);
    }
    if (!hEdit) {
        hEdit = FindWindowEx(hForeground, NULL, L"RICHEDIT", NULL);
    }
    
    return hEdit ? hEdit : hForeground;
}
...
void sendTextDirectUnicode(const std::wstring& text) {
    ...
    // 獲取目標視窗（優先使用焦點的編輯控件）
    HWND hTarget = GetFocusedEditControl();
    HWND hForeground = GetForegroundWindow();
    
    // 如果沒有找到編輯控件，使用前台視窗
    if (!hTarget || hTarget == g_state.hWnd || 
        hTarget == g_state.hCandWnd || hTarget == g_state.hInputWnd ||
        hTarget == g_state.hBufferWnd) {
        hTarget = hForeground;
    }
    ...
    // 嘗試設置焦點到編輯控件
    HWND hEdit = GetFocusedEditControl();
    if (hEdit && hEdit != hTarget) {
        SetFocus(hEdit);
        hTarget = hEdit;
    } else {
        SetFocus(hTarget);
    ...
    // 檢測目標控件類型，決定使用哪種發送方法
    wchar_t className[256] = {0};
    GetClassName(hTarget, className, 256);
    bool useWMChar = false;
    
    // 對於 Scintilla（Notepad++）或其他特殊控件，優先使用 WM_CHAR
    if (wcscmp(className, L"Scintilla") == 0 || 
        wcscmp(className, L"RICHEDIT") == 0 ||
        wcscmp(className, L"RichEdit20A") == 0 ||
        wcscmp(className, L"RichEdit20W") == 0 ||
        wcscmp(className, L"RichEdit50W") == 0) {
    ...
    if (useWMChar) {
        // 方法1：使用 WM_CHAR 消息（適用於 Scintilla、RichEdit 等）
        for (wchar_t ch : text) {
            SendMessage(hTarget, WM_CHAR, (WPARAM)ch, 0);
            Sleep(5);
        }
    } else {
        // 方法2：使用 SendInput（適用於大多數應用程式）
        for (size_t i = 0; i < text.length(); i++) {
            wchar_t ch = text[i];
            
            // 處理高級字符（surrogate pairs）
            if (ch >= 0xD800 && ch <= 0xDBFF && i + 1 < text.length()) {
                // 高代理，需要下一個字符（低代理）
                wchar_t lowSurrogate = text[i + 1];
                if (lowSurrogate >= 0xDC00 && lowSurrogate <= 0xDFFF) {
                    // 使用 SendInput 發送高級字符（surrogate pair）
                    INPUT input[2] = {0};
